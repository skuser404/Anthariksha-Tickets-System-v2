...
const cards = (s: MemberStats): CardDef[] => [
  ...
  { label: 'Rejected', count: s.notConfirmed, icon: XCircle, tone: 'text-rose-500' },
...
export default function MemberDashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Admin notices come first — they are the reason a member is often here. */}
      <AnnouncementBanner />
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        {isLoading || !data
          ? Array.from({ length: 11 }).map((_, i) => <Skeleton key={i} className="h-28" />)
          : cards(data).map((c, i) => (
